...
;
; Code+data size: 0x193 bytes for i686, 0x194 bytes for i386.
;
...
.2:		push ebp
		...
		push edi
		push esi
		cmp ecx, [ebp+8]  ; optind>argc?
		jg .error.jmp
		mov eax, [ebp+0xc]  ; EAX := argument argv.
.again2:	mov edi, [eax+ecx*4]  ; EDI := argv[optind].
		test edi, edi
		jz near .error.jmp
		mov al, '-'
		scasb  ; `cmp [edi], al' ++ `lea edi, [edi+1]'.
		jne near .error.jmp
		mov al, [edi]
		inc edi
		cmp al, '-'
