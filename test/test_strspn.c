...
extern size_t mini_strspn(const char *s, const char *accept);  /* Function under test. */
extern char *mini_index(const char *s, int c);  /* Function under test. */

static char expect(const char *s, const char *accept) {
  size_t expected_value = strspn(s, accept);
  size_t value = mini_strspn(s, accept);
