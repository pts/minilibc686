...
IS_CC1_MEMCPY=  # The value 1 indicates that -fobjc-direct-dispatch must be passed to our patched cc1 (GCC 4.1.2 and GCC 4.2.1) so that it will emit `call __builtin_memcpy' instead of `call memcpy' for autogenerated struct copies with minilibc.
if test -z "$GCC"; then :
elif test "$GCC" = 4; then GCC="$MYDIR"/tools/cc1-4.8.5; IS_CC1=2  # minicc default of the GCC 4.x series.
elif test "$GCC" = 4.1 || test "$GCC" = 4.1.2; then GCC="$MYDIR"/tools/cc1-4.1.2; IS_CC1=2; IS_CC1_MEMCPY=1
elif test "$GCC" = 4.2 || test "$GCC" = 4.2.1; then GCC="$MYDIR"/tools/cc1-4.2.1; IS_CC1=2; IS_CC1_MEMCPY=1
...
if test "$GCC" || test -z "$IS_TCCLD"; then
  ...
  if test "$IS_WATCOM"; then  # If $IS_WATCOM, convert $CCARGS for wcc386.
    ...
    test "$LIBC" = minilibc && test "$IS_CC1_MEMCPY" && CC1ARGS="$CC1ARGS$NL-fobjc-direct-dispatch"  # !! Instruct our patched cc1-4.1.2 and cc1-4.2.1 to emit `call __builtin_memcpy' instead of `call memcpy' for struct assignment.
    ...
    case "$ARG" in
     ...
     --srcfile=?*)
      ...
      if test "$NEED_OCONV" || test "$SFILE"; then
        ...
        if test "$DO_ELFOFIX_FOR_THIS"; then
          EFARGS="$MYDIR/tools/elfofix$NL-r$NL-w$NL$HAD_V$NL--$NL$TMPOFILE2"
...
if test "$DO_SMART" != 0 || test "$SYSNASM"; then  # Smart linking or OS-specific syscall linking.
  ...
  if test "$EC" = 0 && test "$DO_SMART" != 2; then
    ...
    UNDEFSYMS="$(awk '{if((((/: undefined reference to [`'\''].+'\''$/&&sub(/^.*?: undefined reference to [`'\'']/, "")||sub(/^tcc: error: undefined symbol '\''/,""))&&sub(/'\''$/,""))||(/: warning: /&&sub(/^.*: warning: (cannot find )?entry symbol '\''?/,"")&&sub(/['\'';].*/,"")&&!/ /))&&!h[$0]){h[$0]=1;printf"%s%s",s,$0;s=",";++c}}
        END{if(c&&c%16==0){printf",,"}}' <"$OUTFILE.err")"  # ,, to work around segfault-for-16-argument-macro bug in NASM 0.98.39.
