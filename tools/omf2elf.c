...
int main(int argc, char **argv) {
  ...
  FILE *f;
  int fdo = -1;
  char is_verbose = 0, do_strip_one_leading_underscore = 0;
  char is_pass2;
  const char *omfname;
...
#  error This program requires a little-endian system for ELF.  /* Otherwise we would have to do byte order conversion on Elf32_Ehdr etc. */
  ...
  is_pass2 = 0;
  lname_count = seg_count = grp_count = grp_flat = grp_dgroup = shdr_count = elf_file_ofs = extsymbol_count = pubsymbol_count = rel_count = sym_count = 0;
  allnames_size = 1;
 ...
 next_pass:
  ...
  for (;;) {  /* Read subsequent OMF records after the leading THEADR record. */
    ...
    rec_size = g16(rhd + 1);
    if (is_verbose) fprintf(stderr, "info: found OMF record type=0x%02x size+1=0x%x\n", rhd[0], rec_size);
    if (rec_size == 0) {
    ...
    if (rhd[0] == MODEND || rhd[0] == MODEND386) {
      ...
      if (is_verbose) fprintf(stderr, "info: segdef a=%u combine=%u big=%u use32=%u align=%u size=0x%x\n", seg_a, seg_combine, seg_big, seg_use32, seg_align, seg_size);
      if (seg_align == 0) {
      ...
      if (is_verbose) fprintf(stderr, "info: grpdef name=%u(%s)\n", grp_name_idx, lname);
      ...
      /* There are many OMF comment types (i.e. metadata records). Just ignore them. */
      ...
      while (up != data_end) {
        ...
        fix_target &= 3;
        if (0 - 0 && is_verbose) fprintf(stderr, "info: fixupp is_segrel=%d ofs=0x%x f=%d frame=%d t=%d p=%d target=%d\n", fix_is_segrel, fix_ofs, fix_f, fix_frame, fix_t, fix_p, fix_target);
        if (fix_f != 0) {
  ...
  if (!is_pass2) {  /* Transition from pass 1 to pass 2. */
    ...
    *allnamep++ = '\0';  /* To let symbol counting start from 1. */
    ...
    for (u = 0, section = sections; u < SECTION_COUNT; ++u, ++section) {
      ...
      if (section->size != 0 || section->rel_count != 0 || section->sym_count != 0 || section->is_rel_target) ++shdr_count;  /* The regular section. */
  ...
  for (pubsymbolp = pubsymbols; pubsymbolp != pubsymbols_end; ++pubsymbolp) {
    if (!pubsymbolp->is_global) {
      if (is_verbose) fprintf(stderr, "info: found %s #%u (%s)\n", "local", u, pubsymbolp->name);
      if (symp == syms_end) {
       bad_elf_sym_count:
  ...
  for (extsymbolp = extsymbols; extsymbolp != extsymbols_end; ++extsymbolp) {
    if (extsymbolp->prev_idx == (uint32_t)-1) {
      if (is_verbose) fprintf(stderr, "info: found extern #%u (%s)\n", u, extsymbolp->name);
      symp->st_info = ELF32_ST_INFO(STB_GLOBAL, STT_NOTYPE);
  ...
  *up++ = '\0';  /* Start with an empty name. */
  ...
  if (is_verbose) fprintf(stderr, "info: successfully written ELF.o: %s\n", elfoname);
