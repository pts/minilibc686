 ...
 * compilers.
 *
 * !! If -fpc was not specified for wcc386, emit .rodata.c instead of
 *    .rodata.str1.1. Thats because OpenWatcom adds floating point literals
 *    to the CONST section if there was no -fpc.
 */
 ...
 next_pass:
  ...
  for (;;) {  /* Read subsequent OMF records after the leading THEADR record. */
    ...
    if (rhd[0] == MODEND || rhd[0] == MODEND386) {
      ...
      if (up + 6 <= rdata_end && up[0] == 0x80 && up[1] == 0x9b && up[5] == 'c') {
        ...
        if (may_optimize_strings == 2) {
          may_optimize_strings = 1;
          if (is_verbose) fprintf(stderr, "info: may optimize strings (-fpc)\n");
        }
      }
