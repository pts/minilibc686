...
int work(char **argv, char *const envp[]) {
  ...
  for (; *p != '\0'; ++p) {}
  u = p - pathname;
  if (u + sizeof(suffix) > sizeof(pathname2)) return -105;
  memcpy(pathname2, pathname, u);
  memcpy(pathname2 + u, suffix, sizeof(suffix));
  u = q - pathname;
...
#endif
  argv[-2] = pathname;  /* .../.sh */  /* Overwrites the argv pointer argument of main. Hence the separate function (work). */
  argv[-1] = pathname2;  /* argv[0] ++ ".sh". */  /* Overwrites the envp pointer argument of main. Hence the separate function (work). */
  argv[0] = "--sh-script"; 
  /*argv[1] = ...*/ /* $1 of the shell script, kept intact. */
  (void)!execve(pathname, argv - 2, envp);
  return -101;
}
